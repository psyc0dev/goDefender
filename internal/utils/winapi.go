...
type WinAPI struct {
	...
	ProcGetSystemFirmwareTable      *syscall.LazyProc
	ProcGlobalMemoryStatusEx        *syscall.LazyProc
	ProcGetSystemInfo               *syscall.LazyProc
	ProcQueryPerformanceCounter     *syscall.LazyProc
	ProcQueryPerformanceFrequency   *syscall.LazyProc
}
...
func NewWinAPI() *WinAPI {
	...
	w.ProcGetSystemFirmwareTable = w.Kernel32.NewProc("GetSystemFirmwareTable")
	w.ProcGlobalMemoryStatusEx = w.Kernel32.NewProc("GlobalMemoryStatusEx")
	w.ProcGetSystemInfo = w.Kernel32.NewProc("GetSystemInfo")
	w.ProcQueryPerformanceCounter = w.Kernel32.NewProc("QueryPerformanceCounter")
	w.ProcQueryPerformanceFrequency = w.Kernel32.NewProc("QueryPerformanceFrequency")
...
func (w *WinAPI) GetRunningProcessCount() (int, error) {
	names, err := w.GetRunningProcessNames()
	if err != nil {
		return 0, err
	}
...
func (w *WinAPI) GetPhysicalMemoryMB() (uint64, error) {
...
func (w *WinAPI) GetProcessorCount() uint32 {
	var sysInfo SYSTEM_INFO
	w.ProcGetSystemInfo.Call(uintptr(unsafe.Pointer(&sysInfo)))
